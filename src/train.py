...
# Hyperparameters
# ─────────────────────────────────────────────────────────────────
N_QUBITS   = 11     # 3 amplitude + 8 angle (tüm kategorik özellikler)
N_LAYERS   = 2      # StronglyEntanglingLayers depth
BATCH_SIZE = 16
EPOCHS     = 40
LR         = 0.005
...
@qml.qnode(dev, interface="torch")
def hybrid_circuit(x_cont, x_cat3, weights):
    """
    x_cont : tensor of shape (8,)  — continuous features
    x_cat3 : tensor of shape (8,)  — all 8 categorical features
    weights: tensor of shape (N_LAYERS, N_QUBITS, 3)
    returns: list of 11 PauliZ expectations
    """
    qml.AmplitudeEmbedding(x_cont, wires=[0, 1, 2], normalize=True)   # 8 sürekli → 3 kübit
    qml.AngleEmbedding(x_cat3,    wires=range(3, 11), rotation="Y")  # 8 kategorik → 8 kübit
    qml.StronglyEntanglingLayers(weights, wires=range(N_QUBITS))
...
class HybridQNN(nn.Module):
    def __init__(self, n_classes: int = 7):
        super().__init__()
        # Weights are nn.Parameter — gradients flow through qnode directly
        self.q_weights = nn.Parameter(torch.randn(N_LAYERS, N_QUBITS, 3) * 0.01)
        self.head       = nn.Linear(N_QUBITS, n_classes)

    def forward(self, x_cont, x_cat):
        x_cat3 = x_cat                          # (B, 8) — tüm kategorik özellikler
        # Call qnode per sample; torch autograd tracks gradients through loop
        q_out = torch.stack([
            torch.stack(hybrid_circuit(x_cont[i], x_cat3[i], self.q_weights))
...
if __name__ == "__main__":
    ...
    # ── Model, optimiser, scheduler ───────────────────────────
    # Class weights: ters frekans → az örnekli sınıflara daha fazla ağırlık
